<template>
  <div class="offcanvas offcanvas-end" tabindex="-1" id="addClientSimulationCanvas" ref="addClientModal">
    <div class="offcanvTopDiv">
      ...
      <h5 class="fs-38 bold-fw" ref="openModalRef"  data-bs-toggle="offcanvas" data-bs-target="#addClientSimulationCanvas" aria-controls="addClientSimulationCanvas">Add Client</h5>
    </div>
      ...
      <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors }">
        <div class="offcanvasBodyContent">
          ...
          <div class="offcanBtnDiv">
            <input type="submit" class="btn form-control SaveAddBtn" value="Save & Add New Simulations" @click="() => createSimulation = 1">
            <input type="submit" class="btn form-control mt-3 SaveclsBtn" value="Save & Close" @click="() => createSimulation = 0">
          </div>
        </div>
      </Form>
    </div>
    ...
    onSubmit(data, { resetForm }) {
      ...
      post(getUrl("client"), data, authHeader())
        .then(response => {
          ...
          if(this.createSimulation){
             this.$router.push(`/historical/simulation-details?client=${response.data.data.id}`);
          }
        })
        .catch(error => {
          this.$store.dispatch("loader", false);
          this.createSimulation = 0;
