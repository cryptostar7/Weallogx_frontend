<template>
     <div class="offcanvas offcanvas-end" tabindex="-1" id="editClientSimulationCanvas">
    <div class="offcanvTopDiv">
      ...
      <h5 class="fs-38 bold-fw" ref="openModalRef"  data-bs-toggle="offcanvas" data-bs-target="#editClientSimulationCanvas" aria-controls="editClientSimulationCanvas">Edit Client</h5>
    </div>
    ...
    onSubmit(data) {
      ...
      patch(`${getUrl("client")}${this.clientDetail.id}/`, data, authHeader())
        .then(response => {
          ...
          if(this.createSimulations){
            this.createSimulations = false;
            this.$router.push('/historical/simulation-details?client='+this.clientDetail.id);
          }
