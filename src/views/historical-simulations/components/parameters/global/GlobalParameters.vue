<template lang="">
  <div
    class="indexStrategyallDivs active accordion-button collapsed mt-2"
    data-bs-toggle="collapse"
    data-bs-target="#analysis-parameters3"
    aria-expanded="false"
  ...
  >
    <form action="javascript:void(0)" autocomplete="off">
      ...
      <div
        class="ChooseTimePeriodInputs d-flex justify-content-between align-items-center"
        id="rollingTimePeriod1"
      >
        <label
          v-for="(item, index) in rollingTimePeriod"
          :key="index"
          :class="
            item > rollingPeriod.max_val
              ? 'disabled'
              : `${rollingPeriod.custom ? 'disabled' : ''}`
          "
        >
          <input
            type="radio"
            name="radio"
            class="d-none"
            :checked="!rollingPeriod.custom && rollingPeriod.value === item"
            @click="rollingPeriod.value = item"
          />
          <span class="timePeriodYear">{{ item }}</span>
        </label>
        <div class="or-div">or</div>
        <div
          class="customAmountInputDiv customAmountNoPercent creditBonusInputDiv customInputWidth"
        >
          <label
            for="simulationRollingCustomAmount"
            class="position-relative rolling-time-label"
            >Custom Amount
            <img
            ...
            />
            <img
              src="@/assets/images/icons/dark-i-icon.svg"
              alt="info"
              class="dark-info-icon-img"
            />
            <span class="info-message-analyze" id="rollingTimeInfoContent"
              >Choose a rolling period between 15 and 55 years.</span
            >
          </label>
          <input
            id="simulationRollingCustomAmount"
            type="text"
            min="1"
            :max="rollingPeriod.max_val"
            class="bonus-input handleLimit"
            @input="(e) => (rollingPeriod.custom = e.target.value)"
          />
        </div>
      </div>
      ...
      <div class="d-flex justify-content-center align-items-center px-4">
        <div class="IndexStrategyBtn w-100">
          <div class="w-100">
            <input
              type="radio"
              name="CreditBaseMethod1"
              id="monthlyAverageValue"
              :checked="credMethod === 'Monthly Average Value'"
            />
            <label
              for="monthlyAverageValue"
              @click="credMethod = 'Monthly Average Value'"
          ...
          <div class="w-100">
            <input
              ...
              name="CreditBaseMethod1"
              id="beginningValue"
              :checked="credMethod === 'Beginning Value'"
            />
            <label for="beginningValue" @click="credMethod = 'Beginning Value'"
              >Beginning Value</label
            >
          </div>
          <div class="w-100">
            <input
              ...
              name="CreditBaseMethod1"
              id="yearValue"
              :checked="credMethod === 'End of Year Value'"
            />
            <label for="yearValue" @click="credMethod = 'End of Year Value'"
        ...
        <div class="IndexStrategyBtn w-66">
          <div class="w-100">
            <input
              ...
              id="annuallyDist"
              :checked="distributions === 'Annually' ? true : false"
            />
            <label for="annuallyDist" @click="distributions = 'Annually'"
              >Annually
            </label>
          </div>
          <div class="w-100">
            <input
              ...
              name="analysisDistribution"
              id="monthlyDist"
              :checked="distributions === 'Monthly' ? true : false"
            />
            <label for="monthlyDist" @click="distributions = 'Monthly'"
              >Monthly</label
            >
          </div>
        </div>
      ...
      <global-fees-component ref="globalFeesRef" @clearError="clearError" />
      <!-- Fees Global Section End -->
      <input
        type="hidden"
        id="simulation_rolling_time"
        :value="rollingPeriod.custom || rollingPeriod.value"
        ref="rollingRef"
      />
      <input
        type="hidden"
        id="simulation_analyze_type"
        :value="analyze"
        ref="analyzeRef"
      />
      <input
        type="hidden"
        id="simulation_credit_base_method"
        :value="credMethod"
        ref="credRef"
      />
      <input
        type="hidden"
        id="simulation_distribution_method"
        :value="distributions"
...
import GlobalFeesComponent from "../global/GlobalFeesComponent.vue";
import config from "../../../../../services/config.js";
export default {
  components: { SelectDropdown, GlobalFeesComponent },
  emits: ["clearError", "setRollingTime"],
  data() {
    return {
  ...
  methods: {
    ...
    updateRollingPeriod: function (val) {
      ...
      if (
        ...
        document.getElementById("simulationRollingCustomAmount").value =
          this.rollingPeriod.max_val;
    ...
    updateData: function () {
      ...
      this.$refs.globalFeesRef.updateData(); // update global fees data
    },
  },
  computed: {
    indexStrategies() {
