    ...
    >
      <div class="client-with-actions indexSenarioInnerBg">
        <div class="client-name semi-bold-fw fs-18">
          <span class="name-initial-circle">{{
          ...
          >
            <span class="name-span">{{
              $clientName(item.firstname, item.lastname, item.middlename)
            }}</span>
            <svg
              width="9"
              height="12"
            ...
            >
              ...
              <rect
                ...
                height="2.5"
                rx="1.25"
                transform="rotate(135 8.07422 5.76562)"
                fill="black"
              />
            </svg>
          </router-link>
        ...
        <div class="right-action-btns" @click="(e) => e.stopPropagation()">
          ...
          <div class="right-action-btn-div p-relative">
            <button
              type="button"
              data-bs-toggle="collapse"
              :data-bs-target="`#reportCollapse${item.id}${index}`"
              class="btn right-action-btn collapsed"
              aria-expanded="false"
              :id="`reportCollapseBtn${item.id}${index}`"
              @click="handleRef(`${item.id}${index}`, 'report')"
            >
              Reports
              <svg
                width="12"
          ...
          <div class="dropdown three-dots-dropdown">
            <button
              class="btn dropdown-toggle no-after three-dots-btn"
              data-bs-toggle="dropdown"
              aria-expanded="false"
            >
              <img
                src="@/assets/images/icons/three-dots.svg"
                class="img-fluid"
                alt="Three Dots"
              />
            </button>
      ...
      >
        <h4 class="bold-fw fs-22 ScenariosHeadingTxt mb-1">
          Simulations
          <svg
            width="9"
            height="12"
      ...
      <div
        class="list-groups collapse indexSenarioBg"
        :id="`reportCollapse${item.id}${index}`"
        :data-bs-parent="`#parentCollapse${index}`"
        @click="(e) => e.stopPropagation()"
      >
        <h4 class="bold-fw fs-22 reporth4Head mb-1">
          Reports
          <svg
            width="9"
            height="12"
...
export default {
  ...
  components: { SimulationRow, ReportRow },
  data() {
    return {
      senarioLimit: config.SIMULATION_LIST_LIMIT,
      reportLimit: config.SIMULATION_LIST_LIMIT,
    };
  },
  methods: {
