  ...
  <div>
    <div :class="`container large my-5 darkbgClrDiv ${data ? '' : 'd-none'}`">
      <div class="summary-heading">
        ...
        <router-link
          :to="`/historical/illustration-data/${$route.params.simulation}?review=true`"
          ...
          <div class="row">
            ...
            <div class="col-md-2 summaryInputsDiv">
              ...
              <input
                type="text"
                class="form-control"
                :value="data.insurance_policy_name"
                readonly
              />
            </div>
            <div class="col-md-2 summaryInputsDiv">
              ...
              <input
                type="text"
                class="form-control"
                :value="data.insurance_policy_nickname"
                readonly
              />
            </div>
            <div class="col-md-2 summaryInputsDiv">
              ...
              <input
                type="text"
                class="form-control"
                :value="`${Number(data.initial_policy_return || 0).toFixed(
                  2
                )}%`"
                readonly
              />
            ...
            <div class="col-md-2 summaryInputsDiv">
              ...
              <input
                type="text"
                class="form-control"
                :value="`$${Number(data.initial_death_benifit).toLocaleString('en-US')}`"
                readonly
              />
            </div>
            <div
              class="col-md-2 illustrationPdfCol"
              v-for="(item, index) in illustrationFiles"
              :key="index"
            >
              <a
                :href="item.s3_url"
                class="illustrationPdfAncor illustrationPdfDiv btn"
                target="_blank"
              >
                <svg
                  width="18"
        ...
        <div class="container-fluid">
          <div class="row" v-if="illustration">
            <div class="col-md-12 illusnTableMainDiv">
              <div
                id="illustrationScheduleRateTable"
                class="illustrationScheduleRateTable"
                :style="{
                  'overflow-x': 'auto',
                  'overflow-y': 'hidden',
                  width: '100%',
                  height: showAll ? '' : '300px',
                }"
              >
                <table :class="`table ${showAll ? 'showAll' : ''}`">
                  <thead>
                    <th
                      v-for="(header, key) in illustration.headers"
                      :key="key"
                    >
                      {{ illustrationFields[header] }}
                    </th>
                  </thead>
                  <tbody>
                    <tr
                    ...
                    >
                      <td
                        ...
                        :key="i"
                        :data-label="illustrationFields[illustration.headers[i]]"
                      >
                        {{ item }}
                      </td>
                    </tr>
  ...
  methods: {
        ...
        get(`${getUrl("historical-illustrations")}${this.$props.id}/`, authHeader())
          .then((response) => {
            ...
            if (detail.historical_data.copy_paste_checkbox) {
              ...
              this.setIllustrationData(
                detail.historical_data.upload_from_file
              );
            }
          })
          .catch((error) => {
    ...
    getIllsutrationFiles: function () {
      get(getUrl("illustration-files"), authHeader()).then((response) => {
        this.illustrationFiles = response.data.results.filter(
          (i) => i.simulation_id === Number(this.$route.params.simulation)
        );
      });
    },
  },
