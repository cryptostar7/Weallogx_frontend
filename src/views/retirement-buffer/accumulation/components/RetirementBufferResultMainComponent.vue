      ...
      <div class="right-area-wrapper">
        <section class="retirement_buffer_section">
          <div class="retirement_buffer_section_head_div">
            ...
            <router-link to="/retirement-buffer/accumulation/params" class="head_back_btn"
              ><img src="@/assets/images/icons/head-arrow.svg" alt="Arrow" /> Back to Parameters Screen</router-link
            >
          </div>
          <div class="head_all_tabs">
            <div class="nav table_graph_setting_tab nav-pills" role="tablist" aria-orientation="vertical">
              <div
                class="active rb-tab"
                data-bs-toggle="pill"
...
export default {
  ...
  mounted() {
    ...
    selectBtn.forEach((showHide) => {
      showHide.addEventListener('click', () => showHide.closest('.select-menu').classList.toggle('active'));
      ...
      allOptions.forEach((option) => {
        option.addEventListener('click', (e) => {
          ...
          if (option.parentElement) {
            option.parentElement.parentElement.querySelector('.select-btn').querySelector('.sBtn-text').value =
              selectedOption;
  ...
  methods: {
    updateSliderRange: function (modal) {
      ...
      if (obj) {
        ...
        if (!this.marketAlone) {
          marketValue = 100 - Number((obj.buffer_account_allocation * 100).toFixed(0));

          bufferValue = Number((obj.buffer_account_allocation * 100).toFixed(0));
        }
        ...
        if (modal && !this.marketAlone) {
          ...
          this.$refs.formModalRef.$refs.formRef.$refs.sliderRangeRef.setBufferAccountAllocation(bufferValue); // set buffer account allocation value in setting moda slider range
          this.$refs.formModalRef.$refs.formRef.$refs.sliderRangeRef.setMarketAccountAllocation(marketValue); // set market account allocation value in setting moda slider range
        }
      }
    }
  },
  computed: {
    ...
    accountAllocation() {
      ...
      let marketValue = 100 - Number((buffer_account_allocation * 100).toFixed(0));
      ...
      let mav = Number(((account_value / 100) * marketValue).toFixed(0)).toLocaleString('en-US') || 0;
    ...
    years() {
      ...
      if (this.indexType === 'Historical Average') {
        if (this.results.market.net_distribution.length)
          for (let index = 1; index <= this.results.market.net_distribution.length; index++) {
            array.push(index);
          }
      } else {
        array = this.$store.state.data.retirement_buffer.auccumulation_results.years;
      }
      return array;
    }
  },
  watch: {
    marketAlone(e) {
      if (e) {
        ...
        this.$store.dispatch('updateBuffeAccountAllocation', this.inputs.buffer_account_allocation);
      }
    ...
    indexType(e) {
      if (e !== 'Historical Returns') {
        ...
        this.$refs.tableRef.$refs.filterResultRef.getAccumulationResults('average');
      } else {
        if (this.$store.state.data.retirement_buffer.sort_type === 'average') {
          this.$refs.tableRef.$refs.filterResultRef.getAccumulationResults('none');
        }
