      ...
      <div class="row">
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          ...
          <div class="select-menu">
            ...
            <ul class="options">
              <li
                :class="`option ${inputs.rmd_age === 72 ? 'active' : ''}`"
                @click="updateInput('rmd_age', 72)"
              >
                <span class="option-text">72</span>
              </li>
              <li
                :class="`option ${inputs.rmd_age === 73 ? 'active' : ''}`"
                @click="updateInput('rmd_age', 73)"
              >
                <span class="option-text">73</span>
              </li>
              <li
                :class="`option ${inputs.rmd_age === 75 ? 'active' : ''}`"
                @click="updateInput('rmd_age', 75)"
              >
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          <label for="beginningBalance"
            >Switch Year<span
              ><common-tooltip-svg />
              <span>What year should the second tax rate take effect?</span>
            </span></label
          >
          <div
            :class="`index-strategy-each-inputs ${
              switch_year_valid ? '' : 'error'
            }`"
          >
            <integer-input
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          ...
          <div
            :class="`index-strategy-each-inputs ${
              social_security_age_valid ? '' : 'error'
            }`"
          >
            <integer-input
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          <label for="beginningBalance"
            >Social Security COLA <span class="optional-label">(optional) </span
            ><span
              ><common-tooltip-svg />
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          ...
          <div
            id="filingStatus"
            :class="`select-menu ${filing_status_valid ? '' : 'error'} `"
          >
            <div class="select-btn">
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top pb-2">
          <label for="beginningBalance"
            >Add’l Conversion Tax
            <span class="optional-label">(optional)</span>
            <span
              ><common-tooltip-svg />
              <span class="w-250"
                >Additional percentage points of taxation during the conversion
                period. For example, if you are assuming a 20% tax rate but wish
                to assume a 22% tax rate during the conversion period, you would
                enter “2” here.</span
              >
            </span></label
          >
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          ...
          <div id="bonusOnConversion" class="select-menu">
            <div class="select-btn">
              <span class="sBtn-text">{{ inputs.bonus_on_conversion }}</span>
              <i
                ><img
                  src="@/assets/images/icons/select-chevron.svg"
                  alt="Chevron"
            ...
            <ul class="options">
              <li
                :class="`option ${
                  inputs.bonus_on_conversion === 'No' ? 'active' : ''
                }`"
                @click="updateInput('bonus_on_conversion', 'No')"
              >
                <span class="option-text">No</span>
              </li>
              <li
                :class="`option ${
                  inputs.bonus_on_conversion === 'Yes' ? 'active' : ''
                }`"
                @click="updateInput('bonus_on_conversion', 'Yes')"
              >
        ...
        <div class="col-md-6 col-lg-3 inp-mar-top">
          ...
          <div
            :class="`index-strategy-each-inputs ${bonus_amount ? '' : 'error'}`"
          >
            <decimal-input
              @valueUpdated="(v) => updateInput('bonus_amount', v)"
              :default="inputs.bonus_amount"
              max="100"
              placeholder="&nbsp;"
              :disabled="inputs.bonus_on_conversion !== 'Yes'"
            />
            <span class="percent">%</span>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      social_security_age_valid: (state) =>
        state.data.tax_scorecard.validation.social_security_age_valid,
      switch_year_valid: (state) =>
        state.data.tax_scorecard.validation.switch_year_valid,
      filing_status_valid: (state) =>
        state.data.tax_scorecard.validation.filing_status_valid,
      bonus_amount: (state) => state.data.tax_scorecard.validation.bonus_amount,
    }),
  },
  ...
  mounted() {
    ...
    if (this.inputs.filing_status) {
      let text =
        this.inputs.filing_status.charAt(0).toUpperCase() +
        this.inputs.filing_status.slice(1);
      document
        .getElementById("filingStatus")
        .querySelector(".sBtn-text").innerText = text;
    }
    ...
    selectBtn.forEach((showHide) => {
      ...
      allOptions.forEach((option) => {
        option.addEventListener("click", (e) => {
          ...
          if (
            e.currentTarget.closest(".select-menu").getAttribute("id") ==
            "filingStatus"
          ) {
  ...
  methods: {
    validateForm: function (inputs) {
      ...
      if (inputs.bonus_on_conversion === "Yes" && !inputs.bonus_amount) {
        valid = false;
        bonus_amount = false;
      }
      ...
      this.$store.dispatch("updateTaxScorecardFormValidation", {
        form_valid: valid,
        social_security_age_valid: social_security_age_valid,
        switch_year_valid: switch_year_valid,
        filing_status_valid: filing_status_valid,
    ...
    validate_before_request: function (inputs) {
      ...
      if (inputs.second_tax_rate) {
        if (
          inputs.switch_year < 1 ||
          inputs.switch_year > inputs.plan_through_age
        ) {
          errors.push(
            `Switch year must be between 1 and Plan Through Age (${inputs.plan_through_age}).`
          );
        }
      }
      ...
      if (inputs.plan_through_age <= inputs.age) {
        errors.push(
          `Plan Through Age must be greater than Age (${inputs.age}).`
        );
      }
      ...
      if (inputs.social_security_amount) {
        ...
        errors.push(
          `Social Security Age also requires Social Security Amount.`
        );
      }
      ...
      if (inputs.bonus_on_conversion === 'Yes' && !inputs.bonus_amount) {
        errors.push(`Bonus Amount is required.`);
      }
    ...
    generateTaxScorecard: function () {
      ...
      inputs.social_security_cola = this.percentToDecimal(
        inputs.social_security_cola
      );
      inputs.roth_conversion_years = inputs.roth_conversion_years
        ? inputs.roth_conversion_years
        : 0;

      if (inputs.additional_conversion_tax) {
        inputs.additional_conversion_tax = this.percentToDecimal(
          inputs.additional_conversion_tax
        );
      }
      inputs.bonus_amount = this.percentToDecimal(inputs.bonus_amount);
    ...
    resetForm: function () {
      ...
      document
        .getElementById("filingStatus")
        .querySelector(".sBtn-text").innerText = "";
    },
  ...
  watch: {
    "inputs.bonus_on_conversion"(e) {
      if (e !== "Yes") {
        this.updateInput("bonus_amount", "");
      }
    },
  },
};
