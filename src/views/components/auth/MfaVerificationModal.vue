    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title">Enter MFA Code</h3>
...
export default {
  ...
  computed: {
    isCodeValid() {
      return this.mfaCode.length === 6 && /^\d{6}$/.test(this.mfaCode);
    }
  },
  methods: {
    formatCode() {
      // Only allow digits
      this.mfaCode = this.mfaCode.replace(/\D/g, '');
    ...
    async verifyCode() {
      ...
      try {
        ...
        if (error.response && error.response.data) {
          this.error = error.response.data.message || 'Invalid MFA code. Please try again.';
