  ...
  <div>
    <div :class="`container large my-5 darkbgClrDiv ${data ? '' : 'd-none'}`">
      <div class="summary-heading">
        ...
        <router-link
          :to="`/illustration-data/${$route.params.scenario}?review=true`"
        ...
        get(`${getUrl("illustration")}${this.$props.id}/`, authHeader())
          .then((response) => {
            ...
            if (detail.illustration_data.copy_paste_checkbox) {
              this.setIllustrationData(detail.illustration_data.copy_paste);
            } else {
              this.setIllustrationData(
                detail.illustration_data.upload_from_file
              );
            }
          })
          .catch((error) => {
    ...
    getIllsutrationFiles: function () {
      get(getUrl("illustration-files"), authHeader()).then((response) => {
        this.illustrationFiles = response.data.results.filter(
          (i) => i.scenario_id === Number(this.$route.params.scenario)
        );
      });
    },
  },
