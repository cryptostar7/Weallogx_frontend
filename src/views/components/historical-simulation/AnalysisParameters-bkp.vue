...
<div id="analysis-parameters3" class="accordion-collapse collapse analysisParametersContent" data-bs-parent="#accordionFlushExample">
    ...
    <div class="ChooseTimePeriodInputs d-flex justify-content-between align-items-center" id="rollingTimePeriod1">
        <label v-for="(item, index) in rollingTimePeriod" :key="index" :class="item > rollingPeriod.max_val ? 'disabled' : `${rollingPeriod.custom ? 'disabled':''} ${currentTab === 1 ? '' : 'disabled'}`">
        <input type="radio" name="radio" class="d-none" :checked="!rollingPeriod.custom && rollingPeriod.value === item" @click="rollingPeriod.value = item" :disabled="currentTab === 1 ? false : true">
        ...
        <input :id="`rollingCustomAmount${currentTab}`" type="text" min="1" :max="rollingPeriod.max_val" class="bonus-input handleLimit" @input="(e) => rollingPeriod.custom = e.target.value" :disabled="currentTab === 1 ? false : true">
        </div>
    </div>
    ...
    <div class="d-flex justify-content-center align-items-center px-4">
        ...
        <div class="w-100">
            <input type="radio" name="indexStrategy" :id="`index${currentTab}`"  :checked="analyze === 'Index'" :disabled="currentTab === 1 ? false : true">
            <label :for="`index${currentTab}`" @click="() => currentTab === 1 ? analyze = 'Index' : false">Index </label>
        </div>
        <div class="w-100">
            <input type="radio" name="indexStrategy" :id="`strategy${currentTab}`"  :checked="analyze === 'Strategy'" :disabled="currentTab === 1 ? false : true">
            <label :for="`strategy${currentTab}`" @click="() => currentTab === 1 ? analyze = 'Strategy' : false">Strategy </label>
        </div>
        <div class="w-100">
            <input type="radio" name="indexStrategy" :id="`distributions${currentTab}`"  :checked="analyze === 'Distributions'" :disabled="currentTab === 1 ? false : true">
            <label :for="`distributions${currentTab}`" @click="() => currentTab === 1 ? analyze = 'Distributions' : false">Distributions </label>
        </div>
        </div>
    </div>
    <div class="formParabrdrLavelDiv mt-3 mb-2">
    ...
    <div class="d-flex justify-content-center align-items-center px-4">
        <div class="IndexStrategyBtn w-100">
        <div class="w-100">
            <input type="radio" name="CreditBaseMethod1" :id="`monthlyAverageValue${currentTab}`" :checked="credMethod === 'Monthly Average Value'" :disabled="currentTab === 1 ? false : true">
            <label :for="`monthlyAverageValue${currentTab}`" @click="() => currentTab === 1 ? credMethod = 'Monthly Average Value' : false">Monthly Average Value </label>
        </div>
        <div class="w-100">
            <input type="radio" name="CreditBaseMethod1" :id="`beginningValue${currentTab}`" :checked="credMethod === 'Beginning Value'" :disabled="currentTab === 1 ? false : true">
            <label :for="`beginningValue${currentTab}`" @click="() => currentTab === 1 ? credMethod = 'Beginning Value' : false">Beginning Value</label>
        </div>
        <div class="w-100">
            <input type="radio" name="CreditBaseMethod1" :id="`yearValue${currentTab}`" :checked="credMethod === 'End of Year Value'" :disabled="currentTab === 1 ? false : true">
            <label :for="`yearValue${currentTab}`" @click="() => currentTab === 1 ? credMethod = 'End of Year Value' : false">End of Year Value</label>
        </div>
        </div>
    </div>
    <div class="formParabrdrLavelDiv mt-3 mb-2">
    ...
    <div class="d-flex align-items-center px-4">
        ...
        <div class="w-100">
            <input type="radio" :name="`analysisDistribution${currentTab}`" :id="`annuallyDist${currentTab}`" :checked="distributions === 'Annually' ? true : false" :disabled="currentTab === 1 ? false : true">
            <label :for="`annuallyDist${currentTab}`" @click="() => currentTab === 1 ? distributions = 'Annually' : false">Annually </label>
        </div>
        <div class="w-100">
            <input type="radio" :name="`analysisDistribution${currentTab}`" :id="`monthlyDist${currentTab}`" :checked="distributions === 'Monthly' ? true : false" :disabled="currentTab === 1 ? false : true">
            <label :for="`monthlyDist${currentTab}`" @click="() => currentTab === 1 ? distributions = 'Monthly' : false">Monthly</label>
        </div>
        </div>
    </div>
